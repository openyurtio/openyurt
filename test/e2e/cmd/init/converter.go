...
func (c *ClusterConverter) deployYurtManager() error {
	...
	if err := c.ComponentsBuilder.InstallComponents(filepath.Join(c.RootDir, "charts/openyurt/crds"), false); err != nil {
		return err
	}
...
// generatedAutoGeneratedTempFile will replace {{ include "openyurt.namespace" . }} with ns in webhooks
func generatedAutoGeneratedTempFile(root, ns string) (string, error) {
	...
	autoGeneratedYaml := filepath.Join(root, "charts/openyurt/templates/yurt-manager-auto-generated.yaml")
	...
	for i, line := range lines {
		if strings.Contains(line, "{{ include \"openyurt.namespace\" . }}") {
			lines[i] = strings.ReplaceAll(line, "{{ include \"openyurt.namespace\" . }}", ns)
		}
	}
