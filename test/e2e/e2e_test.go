...
var _ = ginkgo.BeforeSuite(func() {
	...
	if labelFilter([]string{"edge-autonomy"}) {
		...
		cs := c
		podSelector := labels.SelectorFromSet(labels.Set(map[string]string{"app": constants.NginxServiceName}))
		ginkgo.By("get nginx pod info on edge node worker2")
		pod, err := p.GetPodByLabelAndNodeName(cs, constants.YurtDefaultNamespaceName, "openyurt-e2e-test-worker2", podSelector)
