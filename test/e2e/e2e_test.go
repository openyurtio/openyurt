...
var _ = ginkgo.BeforeSuite(func() {
	...
	if labelFilter([]string{"edge-autonomy"}) {
		...
		cs := c
		nginxLabel := labels.SelectorFromSet(labels.Set(map[string]string{"app": constants.NginxServiceName}))
		ginkgo.By("get nginx pod info on node openyurt-e2e-test-worker2")
		pod, err := p.GetPodByLabelAndNode(cs, constants.YurtDefaultNamespaceName, nginxLabel, "openyurt-e2e-test-worker2")
