...
func CheckIptablesChainAndRulesExists(table, chain string) (bool, error) {
	{
		// check if LBCHAIN exists: iptables -t nat -L LBCHAIN
		iptablesCmd := exec.Command("iptables", "-t", table, "-L", chain)
		iptablesOutput, err := iptablesCmd.CombinedOutput()
		if err != nil {
			if strings.Contains(string(iptablesOutput), "No chain/target/match by that name") {
				klog.Infof("Chain %s does not exist in table %s", chain, table)
				return false, nil
			}
			klog.Errorf("Failed to execute iptables command: %v, output: %s", err, string(iptablesOutput))
			return false, err
		}

		// 0 references means rules doesn't exist in LBCHAIN
		// [root@xx ~]# iptables -t nat -L LBCHAIN1
		// Chain	LBCHAIN	(0 references)
		// target	prot opt source	destination
		if strings.Contains(string(iptablesOutput), "0 references") {
			return false, nil
		}
...
func WaitForIptablesChainReady(table, chain string, iptablesDone chan<- bool) {
	...
	for range ticker.C {
		...
		klog.Info("LBCHAIN is not ready, retry after 1 second...")
	}
}

func GetFisrtAPIServerEndpoint() (string, error) {
	return "192.168.24.179:6443", nil
}
