...
func newJoinData(args []string, opt *joinOptions) (*joinData, error) {
	...
	iptablesDone := make(chan bool)
	go localnode.WaitForIptablesChainReady("nat", "LBCHAIN", iptablesDone)
	<-iptablesDone
	klog.Infof("LBCHAIN exists, continue to join node")

	// join the node to the first apiserver endpoints, which equal to joining the node to muti apiservers cluster
	data.apiServerEndpoint, err = localnode.GetFisrtAPIServerEndpoint()
	if err != nil {
		klog.Errorf("could not get apiserver endpoint, %v", err)
		return nil, err
	}
