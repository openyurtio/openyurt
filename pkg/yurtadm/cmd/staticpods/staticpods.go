...
import (
	...
	"github.com/openyurtio/openyurt/pkg/yurtadm/cmd/staticpods/install"
	"github.com/openyurtio/openyurt/pkg/yurtadm/cmd/staticpods/uninstall"
	util "github.com/openyurtio/openyurt/pkg/yurtadm/util/error"
)

// NewCmdStaticPods returns "yurtadm staticpods" command.
func NewCmdStaticPods(in io.Reader, out io.Writer, outErr io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "staticpods",
		Short: "Install or uninstall static pods",
		// Without this callback, if a user runs just the "staticpods"
