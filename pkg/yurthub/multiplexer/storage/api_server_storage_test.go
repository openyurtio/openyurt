...
var (
	...
	fooGV            = schema.GroupVersion{Group: "samplecontroller.k8s.io", Version: "v1alpha1"}
	fooCodec         = unstructured.UnstructuredJSONScheme
)

func TestRestStore_GetList(t *testing.T) {
	t.Run(" list services", func(t *testing.T) {
		rs := &apiServerStorage{
			restClient: newFakeClient(corev1GV, mockServiceListBody(), newListHeader(), false),
		}
	...
	t.Run("list foo", func(t *testing.T) {
		...
		getListObj.SetGroupVersionKind(schema.GroupVersionKind{
			Group:   fooGV.Group,
			Version: fooGV.Version,
			Kind:    "FooList",
		})
		err := rs.GetList(context.Background(), "", storage.ListOptions{}, getListObj)
...
func mockFooListBody() []byte {
	str := runtime.EncodeOrDie(fooCodec, newFooList())
	return []byte(str)
}
...
func newFooList() *unstructured.UnstructuredList {
...
func newFakeClient(gv schema.GroupVersion, body []byte, header http.Header, isCRD bool) *fake.RESTClient {
	...
	if isCRD {
		client.NegotiatedSerializer = serializer.NewUnstructuredNegotiatedSerializer()
	} else {

		client.NegotiatedSerializer = scheme.Codecs.WithoutConversion()
	}
...
func TestRestStore_Watch(t *testing.T) {
	rs := &apiServerStorage{
		restClient: newFakeClient(corev1GV, mockServiceWatchBody(), newWatchHeader(), false),
	}
