...
const (
	webhookName = "NodePool-mutate-webhook"
)
...
func (h *NodePoolCreateUpdateHandler) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	// Set defaults
	appsv1alpha1.SetDefaultsNodePool(obj)
	...
	if len(resp.Patches) > 0 {
		klog.Infof(Format("Admit NodePool %s patches: %v", obj.Name, util.DumpJSON(resp.Patches)))
	}
