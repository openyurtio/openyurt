...
const (
	webhookName = "StaticPod-mutate-webhook"
)
...
func (h *StaticPodCreateUpdateHandler) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	// Set defaults
	appsv1alpha1.SetDefaultsStaticPod(obj)
	...
	if len(resp.Patches) > 0 {
		klog.Infof(Format("Admit StaticPod %s patches: %v", obj.Name, util.DumpJSON(resp.Patches)))
	}
