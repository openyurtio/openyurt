...
func (webhook *IoTHandler) Default(ctx context.Context, obj runtime.Object) error {
	iot, ok := obj.(*v1alpha2.IoT)
	if !ok {
		return apierrors.NewBadRequest(fmt.Sprintf("expected a IoT but got a %T", obj))
	}

	v1alpha2.SetDefaultsIoT(iot)

	if iot.Spec.Version == "" {
		iot.Spec.Version = webhook.Manifests.LatestVersion
	}

	if iot.Spec.Platform == "" {
		iot.Spec.Platform = v1alpha2.IoTPlatformEdgeX
	}
