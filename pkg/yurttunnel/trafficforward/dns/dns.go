...
const (
	...
	dnatPortPrefix          = "dnat-"
	tunnelDNSControllerName = "tunnel-dns-controller"
)
...
func (dnsctl *coreDNSRecordController) Run(stopCh <-chan struct{}) {
	...
	rl, err := resourcelock.New("leases", metav1.NamespaceSystem, tunnelDNSControllerName,
...
func (dnsctl *coreDNSRecordController) run(stopCh <-chan struct{}) {
	...
	if !cache.WaitForNamedCacheSync(tunnelDNSControllerName, stopCh,
