...
func (webhook *YurtAppOverriderHandler) ValidateCreate(ctx context.Context, obj runtime.Object) (admission.Warnings, error) {
	...
	if !ok {
		return nil, apierrors.NewBadRequest(fmt.Sprintf("expected a YurtAppOverrider but got a %T", obj))
	}
...
func (webhook *YurtAppOverriderHandler) ValidateUpdate(ctx context.Context, oldObj, newObj runtime.Object) (admission.Warnings, error) {
	...
	if newOverrider.Subject.Kind != oldOverrider.Subject.Kind || newOverrider.Subject.Name != oldOverrider.Subject.Name {
		return nil, fmt.Errorf("unable to modify subject after %s is created", oldOverrider.Name)
	}
	// validate
...
func (webhook *YurtAppOverriderHandler) ValidateDelete(ctx context.Context, obj runtime.Object) (admission.Warnings, error) {
	...
	case "YurtAppSet":
		...
		if err == nil {
			return nil, fmt.Errorf("namespace: %s, unable to delete YurtAppOverrider when subject resource exists: %s", overrider.Namespace, appSet.Name)
		}
	case "YurtAppDaemon":
		...
		if err == nil {
			return nil, fmt.Errorf("namespace: %s, unable to delete YurtAppOverrider when subject resource exists: %s", overrider.Namespace, appDaemon.Name)
		}
	}
	return nil, nil
}
