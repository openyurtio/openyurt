...
func TestDefaultCompatibleWithV1(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err == nil {
				assert.Equal(t, tc.version, tc.obj.(*v1alpha2.PlatformAdmin).Spec.Version)
			}
		})
	}
}
...
func TestDefaultV2(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			_ = handler.Default(context.TODO(), tc.obj)
			assert.Equal(t, tc.platform, tc.obj.(*v1alpha2.PlatformAdmin).Spec.Platform)
		})
	}
}
