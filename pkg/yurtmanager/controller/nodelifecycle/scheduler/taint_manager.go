...
func deletePodHandler(c client.Client, emitEventFunc func(types.NamespacedName)) func(ctx context.Context, args *WorkArgs) error {
	return func(ctx context.Context, args *WorkArgs) error {
		...
		klog.Infof("NoExecuteTaintManager is deleting pod %s", args.NamespacedName.String())
...
func NewNoExecuteTaintManager(recorder record.EventRecorder, cacheClient client.Client, getPodsAssignedToNode GetPodsByNodeNameFunc) *NoExecuteTaintManager {
...
func (tc *NoExecuteTaintManager) Run(ctx context.Context) {
	...
	klog.Info("Starting NoExecuteTaintManager")
...
func (tc *NoExecuteTaintManager) processPodOnNode(
	...
	if !allTolerated {
		klog.V(2).Infof("Not all taints are tolerated after update for pod(%s) on node(%s)", podNamespacedName.String(), nodeName)
	...
	if minTolerationTime < 0 {
		klog.V(4).Infof("Current tolerations for pod(%s) tolerate forever, cancelling any scheduled deletion", podNamespacedName.String())
		tc.cancelWorkWithEvent(podNamespacedName)
		return
	}
...
func (tc *NoExecuteTaintManager) handlePodUpdate(ctx context.Context, podUpdate podUpdateItem) {
	...
	var pod v1.Pod
	err := tc.cacheClient.Get(ctx, types.NamespacedName{Namespace: podUpdate.podNamespace, Name: podUpdate.podName}, &pod)
	if err != nil {
		if apierrors.IsNotFound(err) {
			// Delete
			podNamespacedName := types.NamespacedName{Namespace: podUpdate.podNamespace, Name: podUpdate.podName}
			klog.V(4).Infof("Noticed pod(%s) deletion", podNamespacedName.String())
			tc.cancelWorkWithEvent(podNamespacedName)
			return
		}
	...
	klog.V(4).Infof("Noticed pod(%s) update", podNamespacedName.String())
...
func (tc *NoExecuteTaintManager) handleNodeUpdate(ctx context.Context, nodeUpdate nodeUpdateItem) {
	...
	node := new(v1.Node)
	err := tc.cacheClient.Get(ctx, types.NamespacedName{Name: nodeUpdate.nodeName}, node)
	if err != nil {
		if apierrors.IsNotFound(err) {
			// Delete
			klog.V(4).Infof("Noticed node(%s) deletion", nodeUpdate.nodeName)
			tc.taintedNodesLock.Lock()
			defer tc.taintedNodesLock.Unlock()
	...
	// Create or Update
	klog.V(4).Infof("Noticed node(%s) update", node.Name)
	taints := getNoExecuteTaints(node.Spec.Taints)
	func() {
		tc.taintedNodesLock.Lock()
		defer tc.taintedNodesLock.Unlock()
		klog.V(4).Infof("Updating known taints on node(%s), %v", node.Name, taints)
		if len(taints) == 0 {
	...
	if len(taints) == 0 {
		klog.V(4).Infof("All taints were removed from the node(%s). Cancelling all evictions...", node.Name)
		for i := range pods {
