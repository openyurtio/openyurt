...
func transToV1Alpha2Condition(srcConditions []v1beta1.PlatformAdminCondition) []PlatformAdminCondition {
	if srcConditions == nil {
		return nil
	}

	dstConditions := make([]PlatformAdminCondition, len(srcConditions))
	for i, condition := range srcConditions {
		dstConditions[i] = PlatformAdminCondition{
...
func transToV1Beta1Condition(srcConditions []PlatformAdminCondition) []v1beta1.PlatformAdminCondition {
	if srcConditions == nil {
		return nil
	}

	dstConditions := make([]v1beta1.PlatformAdminCondition, len(srcConditions))
	for i, condition := range srcConditions {
		dstConditions[i] = v1beta1.PlatformAdminCondition{
