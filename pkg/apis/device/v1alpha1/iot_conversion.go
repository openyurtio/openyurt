...
func (src *IoT) ConvertTo(dstRaw conversion.Hub) error {
	// Transform metadata
	dst := dstRaw.(*v1alpha2.IoT)
	...
	dst.Spec.Platform = v1alpha2.IoTPlatformEdgeX
...
func (dst *IoT) ConvertFrom(srcRaw conversion.Hub) error {
	// Transform metadata
	src := srcRaw.(*v1alpha2.IoT)
	dst.ObjectMeta = src.ObjectMeta
	dst.TypeMeta = src.TypeMeta
...
func transToV1Condition(c2 []v1alpha2.IoTCondition) (c1 []IoTCondition) {
	for _, ic := range c2 {
		c1 = append(c1, IoTCondition{
			Type:               IoTConditionType(ic.Type),
			Status:             ic.Status,
...
func transToV2Condition(c1 []IoTCondition) (c2 []v1alpha2.IoTCondition) {
	for _, ic := range c1 {
		c2 = append(c2, v1alpha2.IoTCondition{
			Type:               v1alpha2.IoTConditionType(ic.Type),
			Status:             ic.Status,
			LastTransitionTime: ic.LastTransitionTime,
			Reason:             ic.Reason,
			Message:            ic.Message,
