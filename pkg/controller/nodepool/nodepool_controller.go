...
func init() {
	flag.IntVar(&concurrentReconciles, "nodepool-workers", concurrentReconciles, "Max concurrent workers for NodePool controller.")
}

var (
	concurrentReconciles = 3
	controllerKind       = appsv1alpha1.SchemeGroupVersion.WithKind("NodePool")
)
...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	err = c.Watch(&source.Kind{Type: &appsv1alpha1.NodePool{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}
...
func (r *ReconcileNodePool) Reconcile(_ context.Context, request reconcile.Request) (reconcile.Result, error) {

	// Note !!!!!!!!!!
	// We strongly recommend use Format() to  encapsulation because Format() can print logs by module
	// @kadisi
	klog.Infof(Format("Reconcile NodePool %s/%s", request.Namespace, request.Name))

	// Fetch the NodePool instance
	instance := &appsv1alpha1.NodePool{}
	...
	if instance.Spec.Foo != instance.Status.Foo {
		...
		if err = r.Status().Update(context.TODO(), instance); err != nil {
			klog.Errorf(Format("Update NodePool Status %s error %v", klog.KObj(instance), err))
	...
	//	klog.Errorf(Format("Update NodePool %s error %v", klog.KObj(instance), err))
	//	return reconcile.Result{Requeue: true}, err
	//}
