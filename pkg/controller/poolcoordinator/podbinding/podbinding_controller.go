...
func (r *ReconcilePodBinding) configureTolerationForPod(pod *corev1.Pod, tolerationSeconds *int64) error {
	...
	if toleratesNodeNotReady || toleratesNodeUnreachable {
		klog.V(4).Infof(Format("pod(%s/%s) => toleratesNodeNotReady=%+v, toleratesNodeUnreachable=%+v", pod.Namespace, pod.Name, toleratesNodeNotReady, toleratesNodeUnreachable))
