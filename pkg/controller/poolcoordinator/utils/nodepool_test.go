...
func TestNodeNodepool(t *testing.T) {
	...
	node.Labels[constant.LabelKeyNodePool] = "ut"
	pool, ok := NodeNodepool(node)
	if ok == false || pool != "ut" {
		t.Fail()
	}
}

func TestNodeIsInAutonomy(t *testing.T) {
	node := &corev1.Node{}
	node.Annotations = map[string]string{}
	node.Annotations[constant.AnnotationKeyNodeAutonomy] = "true"
	if NodeIsInAutonomy(node) != true {
		t.Fail()
	}
