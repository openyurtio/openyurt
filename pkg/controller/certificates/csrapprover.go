...
var (
	...
	clientServerRequiredUsages = sets.NewString(
		string(certificatesv1.UsageKeyEncipherment),
		string(certificatesv1.UsageDigitalSignature),
		string(certificatesv1.UsageClientAuth),
		string(certificatesv1.UsageServerAuth))
	recognizers = []csrRecognizer{
		...
		{
			recognize:  isYurtHubServerCert,
			successMsg: "Auto approving yurthub server certificate",
		},
		{
			recognize:  isYurtTunnelServerCert,
			successMsg: "Auto approving yurt-tunnel-server certificate",
		},
		{
...
// isYurtHubServerCert is used to recognize csr for yurthub https server that listens requests from edge clients like kubelet/kube-proxy.
func isYurtHubServerCert(csr *certificatesv1.CertificateSigningRequest, x509cr *x509.CertificateRequest) bool {
...
func isYurtTunnelServerCert(csr *certificatesv1.CertificateSigningRequest, x509cr *x509.CertificateRequest) bool {
	...
	if len(x509cr.Subject.Organization) != 2 {
		...
		for _, org := range x509cr.Subject.Organization {
			if org != certmanager.YurtTunnelCSROrg && org != user.SystemPrivilegedGroup {
				return false
			}
		}
	}

	if x509cr.Subject.CommonName != certmanager.YurtTunnelServerCSRCN {
		return false
	}

	if !clientServerRequiredUsages.Equal(usagesToSet(csr.Spec.Usages)) {
		return false
	}
