...
/*
This file was derived from k8s.io/kubernetes/pkg/controller/nodelifecycle/node_lifecycle_controller_test.go
at commit: 27522a29feb.

CHANGELOG from OpenYurt Authors:
1. Remove Test_isNodeExcludedFromDisruptionChecks.
...
func TestMonitorNodeHealthEvictPodsWithDisruption(t *testing.T) {
	...
	table := []struct {
		...
		// OpenYurt Authors:
		// We don't use master node role to determine whether exclude this node from disruption check in
		// func isNodeExcludedFromDisruptionChecks in node_lifecycle_controller.go. So, do not run this case.
...
func TestReconcileNodeLabels(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// OpenYurt Authors:
			// Currently in node_lifecycle_controller.go, we have
			// - primaryKey: "beta.kubernetes.io"
			// - secondaryKey: "kubernetes.io"
			// But in node_lifecycle_controller_test.go, which is from k8s 1.20.11,
			// assuming that
			// - primaryKey: "kubernetes.io"
			// - secondaryKey: "beta.kubernetes.io"
			// So we change this case.
