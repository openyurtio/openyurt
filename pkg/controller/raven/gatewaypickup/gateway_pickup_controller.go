...
func (r *ReconcileGateway) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	// 2. config gateway enabled service
	err = r.configEndpoints(ctx, &gw)
	if err != nil {
		klog.Error(Format("failed to config endpoints for gateway %s, error %s", gw.GetName(), err.Error()))
		return reconcile.Result{Requeue: true}, err
	}

	// 3. get nodeInfo list of nodes managed by the Gateway
...
func (r *ReconcileGateway) recordEndpointEvent(sourceObj *ravenv1alpha1.Gateway, previous, current []*ravenv1alpha1.Endpoint) {
...
func (r *ReconcileGateway) electActiveEndpoint(nodeList corev1.NodeList, gw *ravenv1alpha1.Gateway) []*ravenv1alpha1.Endpoint {
	...
	eps = append(eps, electEndpoints(gw, ravenv1alpha1.Tunnel, readyNodes)...)
	eps = append(eps, electEndpoints(gw, ravenv1alpha1.Proxy, readyNodes)...)
	sort.Slice(eps, func(i, j int) bool { return eps[i].NodeName < eps[j].NodeName })
	return eps
}

func electEndpoints(gw *ravenv1alpha1.Gateway, endpointType string, readyNodes map[string]*corev1.Node) []*ravenv1alpha1.Endpoint {
	eps := make([]*ravenv1alpha1.Endpoint, 0)
	var replicas int
	switch endpointType {
	case ravenv1alpha1.Proxy:
		replicas = gw.Spec.ProxyConfig.Replicas
	case ravenv1alpha1.Tunnel:
...
func (r *ReconcileGateway) configEndpoints(ctx context.Context, gw *ravenv1alpha1.Gateway) error {
	var cm corev1.ConfigMap
	err := r.Get(ctx, client.ObjectKey{Name: utils.RavenGlobalConfig, Namespace: utils.WorkingNamespace}, &cm)
	if err != nil && !apierrs.IsNotFound(err) {
		return err
	}
	var enableProxy, enableTunnel bool
	if !apierrs.IsNotFound(err) && cm.GetDeletionTimestamp() == nil {
		if val, ok := cm.Data[utils.RavenEnableProxy]; ok && strings.ToLower(val) == "true" {
			enableProxy = true
		}
		if val, ok := cm.Data[utils.RavenEnableTunnel]; ok && strings.ToLower(val) == "true" {
			enableTunnel = true
		}
	}
	getPorts := func(s string) map[string]struct{} {
		ports := make(map[string]struct{})
		for _, v := range strings.Split(s, ",") {
			ports[strings.TrimSpace(v)] = struct{}{}
		}
		return ports
	}
	getPort := func(ports map[string]struct{}) string {
		for k := range ports {
			delete(ports, k)
			return k
		}
	...
	proxyServerPorts := getPorts(gw.Spec.ProxyConfig.ProxyServerPort)
	VPNServerPorts := getPorts(gw.Spec.TunnelConfig.VPNServerPort)
	...
	for idx, val := range gw.Status.ActiveEndpoints {
		...
		case ravenv1alpha1.Proxy:
			...
			if port, ok := gw.Status.ActiveEndpoints[idx].Config[utils.ProxyServerPort]; ok {
				if _, ok := proxyServerPorts[port]; ok {
					delete(proxyServerPorts, port)
					continue
				}
			}
			gw.Status.ActiveEndpoints[idx].Config[utils.ProxyServerPort] = getPort(proxyServerPorts)

		case ravenv1alpha1.Tunnel:
			gw.Status.ActiveEndpoints[idx].Config[utils.RavenEnableTunnel] = strconv.FormatBool(enableTunnel)
			if port, ok := gw.Status.ActiveEndpoints[idx].Config[utils.VPNServerPort]; ok {
				if _, ok = VPNServerPorts[port]; ok {
					delete(VPNServerPorts, port)
					continue
				}
			}
			gw.Status.ActiveEndpoints[idx].Config[utils.VPNServerPort] = getPort(VPNServerPorts)
		default:
...
func getActiveEndpointsInfo(eps []*ravenv1alpha1.Endpoint) (map[string][]string, int) {
