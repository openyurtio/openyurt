...
/*
This file was derived from k8s.io/kubernetes/pkg/controller/testutil/test_utils.go
at commit: 27522a29feb.

CHANGELOG from OpenYurt Authors:
1. Replace FakeNodeHandler.Create with an empty function
2. Remove FakeNodeHandler.GetUpdatedNodesCopy
3. Remove function GetKey
4. Remove keyFunc
5. Add empty functions: FakeNodeHandler.Apply, FakeNodeHandler.ApplyStatus
6. Use runtime.NewScheme() instead legacyscheme.Scheme in FakeRecorder.generateEvent
*/
...
// TODO:
// We add this empty function, because the code of test_utils.go is from kubernetes v1.20.11, while the
// client-go is v0.22.3. We can fulfill it when we bump our controller code to v1.22.3.
func (m *FakeNodeHandler) Apply(_ context.Context, node *corev1.NodeApplyConfiguration, opts metav1.ApplyOptions) (result *v1.Node, err error) {
	return nil, nil
}
...
// We add this empty function, because the code of test_utils.go is from kubernetes v1.20.11, while the
// client-go is v0.22.3. We can fulfill it when we bump our controller code to v1.22.3.
func (m *FakeNodeHandler) ApplyStatus(ctx context.Context, node *corev1.NodeApplyConfiguration, opts metav1.ApplyOptions) (result *v1.Node, err error) {
	return nil, nil
}
...
func (f *FakeRecorder) generateEvent(obj runtime.Object, timestamp metav1.Time, eventtype, reason, message string) {
	...
	ref, err := ref.GetReference(legacyScheme, obj)
	if err != nil {
